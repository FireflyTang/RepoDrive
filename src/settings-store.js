...
function preset() {
  try {
    ...
    return { repository: value.repository, branch: value.branch || 'main', rootPath: value.rootPath || '' };
  } catch { return null; }
}
...
function read() {
  ...
  const fixed = preset();
  return { ...defaults, ...stored, ...(fixed || {}), fixedRepository: Boolean(fixed) };
}

function getPublic() {
  const data = read();
  return { repository: data.repository, branch: data.branch, rootPath: data.rootPath, proxy: data.proxy, hasToken: Boolean(data.tokenEncrypted), fixedRepository: data.fixedRepository };
}
...
function save(next) {
  ...
  const data = { repository: current.fixedRepository ? current.repository : next.repository, branch: current.fixedRepository ? current.branch : next.branch, rootPath: current.fixedRepository ? current.rootPath : next.rootPath, proxy: next.proxy, tokenEncrypted };
