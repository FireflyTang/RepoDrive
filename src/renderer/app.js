...
function appendNodes(parentPath, depth, fragment) {
  for (const item of state.tree.get(parentPath) || []) {
    ...
    const actions = document.createElement('div'); actions.className = 'file-actions'; actions.append(actionButton('download', '下载', () => download(item)), actionButton('trash', '删除', () => remove(item), true));
...
async function init() {
  ...
  $('platformNote').textContent = info.canUpload ? `${info.platform === 'linux' ? 'Linux' : 'macOS'} 完整模式 · 可浏览、下载、上传与删除` : 'Windows 无上传模式 · 可浏览、下载与删除';
