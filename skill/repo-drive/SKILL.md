...
Use the bundled `scripts/repodrive` launcher. The installed RepoDrive CLI reads its fixed `repodrive.config.json`; do not ask the user to repeat the repository and do not pass `--repo` unless the user explicitly requests a one-off override.
...
- Upload on macOS: first list the remote parent and obtain user confirmation, then run `scripts/repodrive upload <local-file-or-directory> --path <remote-parent> --json`. Directories retain their name and hierarchy, skip `.git`, and are not compressed.
