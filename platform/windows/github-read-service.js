...
class GitHubReadService {
  ...
  async request(apiPath, options = {}) {
    ...
    const headers = { Accept: options.raw ? 'application/vnd.github.raw+json' : 'application/vnd.github+json', 'X-GitHub-Api-Version': '2022-11-28', 'User-Agent': 'RepoDrive-Windows/0.4', ...(config.token ? { Authorization: `Bearer ${config.token}` } : {}) };
