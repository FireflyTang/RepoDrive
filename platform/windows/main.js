...
function register(){
  ...
  ipcMain.handle('settings:save',async(_event,form)=>{const previous={...config()};const saved=settings.publicSettings();const repository=saved.fixedRepository?saved.repository:form.repository;const parsed=parseRepository(repository);const candidate={repository:`${parsed.owner}/${parsed.repo}`,branch:validateBranch(saved.fixedRepository?saved.branch:form.branch),rootPath:normalizeRepoPath(saved.fixedRepository?saved.rootPath:form.rootPath),proxy:validateProxy(form.proxy),token:String(form.token||'').trim()||(form.clearToken?'':settings.token()),clearToken:Boolean(form.clearToken),...parsed};runtime=candidate;try{await applyProxy(candidate.proxy);const repo=await github.validateRepository();const stored=settings.save(candidate);runtime={...stored,...parsed,token:candidate.token};return{settings:stored,repo};}catch(error){runtime=previous;await applyProxy(previous.proxy).catch(()=>{});throw error;}});
